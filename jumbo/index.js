...
async function run() {
  ...
  for (let loopIndex = loopStart; loopIndex < totalLoops; loopIndex++) {
    try {
      ...
      const scriptTag = await bodyTag.$('script[type="application/ld+json"]');
      const barcodeContent = await page.evaluate(
        (element) => element.textContent,
        scriptTag,
      );
      const barcode = JSON.parse(barcodeContent);
      console.log(barcode);
      ...
      const weightContent = await page?.evaluate(
        (element) => element.textContent,
        weightElement,
      );

      const allergensElement = await page.$(
        'div[data-testid="allergens-content"]',
      );
      ...
      const categoryContext = await page.$eval(
        "ol.breadcrumb-trail li:first-child",
